/*
   Copyright 2011 Print Printdev@googlemail.com
        ...
        public PrinterConnection()
        {
            ...
            try
            {
                ...
                if (dir.Length > 0 && Main.globalSettings.LogEnabled)
                    logWriter = new StreamWriter(dir + Path.DirectorySeparatorChar + "Print.log");
            }
            catch
            {
                logWriter = null;
       ...
       /* private void StoreHistory(GCode gcode)
        ...
        public void ResendLine(int line)
        {
            ...
            if (resendError > 5 || node == null)
            {
                ...
                close();
                PrintHost.view.SoundConfig.PlayError(false);
        ...
        public void open()
        {
            ...
            isMarlin = isPrint = isSprinter = false;
        ...
        public void analyzeResponse(string res,ref int level)
        {
            ...
            if (h != null)
            {
                ...
                firmware = h;
                h = h.ToLower();
                if (h.IndexOf("Print") >= 0)
                {
                    isPrint = true;
                }
                if (h.IndexOf("marlin") >= 0) isMarlin = true;
                if (h.IndexOf("sprinter") >= 0) isSprinter = true;
                if (isMarlin || isPrint || isSprinter) // Activate special menus and function
                {
