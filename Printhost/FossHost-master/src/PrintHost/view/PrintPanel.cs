    ...
    public partial class PrintPanel : UserControl
    {
        ...
        public void UpdateConStatus(bool c)
        {
           ...
           // switchConnect.On = c;
            ...
            sliderSpeed.Enabled = c && (con.isMarlin || con.isPrint || con.isSprinter);
            sliderFlowrate.Enabled = c && (con.isMarlin || con.isPrint || con.isSprinter);
            numericUpDownSpeed.Enabled = c && (con.isMarlin || con.isPrint || con.isSprinter);
            numericUpDownFlow.Enabled = c && (con.isMarlin || con.isPrint || con.isSprinter);
            if (c) sendDebug();
        }
        ...
        private void sliderSpeed_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (oldcon != con.speedMultiply && con.connector.IsConnected() && (con.isMarlin || con.isPrint || con.isSprinter))
            {
                con.ignoreFeedback();
        ...
        private void sliderFlowrate_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (oldcon != con.flowMultiply && con.connector.IsConnected() && (con.isMarlin || con.isPrint || con.isSprinter))
            {
                con.ignoreFeedback();
