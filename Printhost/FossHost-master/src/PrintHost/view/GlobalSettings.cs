...
namespace PrintHost.view
{
    public partial class GlobalSettings : Form
    {
        ...
        {
            PrintKey.SetValue("workdir", Workdir);
            PrintKey.SetValue("logEnabled", LogEnabled ? 1 : 0);
            PrintKey.SetValue("disableQualityReduction", DisableQualityReduction ? 1 : 0);
            PrintKey.SetValue("reduceToolbarSize", ReduceToolbarSize ? 1 : 0);
            RegMemory.SetInt("onOffImageOffset", checkRedGreenSwitch.Checked ? 2 : 0);
        }
        public void RegToForm()
        {
            Workdir = (string)PrintKey.GetValue("workdir", Workdir);
            checkLogfile.Checked = 1== (int) PrintKey.GetValue("logEnabled", LogEnabled ? 1 : 0);
            checkDisableQualityReduction.Checked = 1 == (int)PrintKey.GetValue("disableQualityReduction", DisableQualityReduction ? 1 : 0);
            checkReduceToolbarSize.Checked = 1 == (int)PrintKey.GetValue("reduceToolbarSize", ReduceToolbarSize ? 1 : 0);
            checkRedGreenSwitch.Checked = 2 == RegMemory.GetInt("onOffImageOffset", 0);
        }
        public static void Associate(string extension,
           string progID, string description)
        {
            string icon = Application.StartupPath + Path.DirectorySeparatorChar + "Print-logo-trans32.ico";
